...
export async function updateTable(reservationToUpdate, signal,id) {
  const url = `${API_BASE_URL}/tables/${id}/seat`;
  
  const options = {
    method: "PUT",
    headers,
    body: JSON.stringify({ data: reservationToUpdate }),
    signal,
  };
  return await fetchJson(url, options, reservationToUpdate);
}
